...
impl ChatWidget {
    pub(super) fn handle_adaptive_runtime_signal(
        ...
        // report_adaptive_signal is transported as its own app-server notification. Live traffic
        // can observe that notification immediately after turn/completed even though the completed
        // turn already contains the trusted tool output proving the report was emitted. Admit only
        // that exact late delivery for the same thread/turn; generic post-turn signals stay inert.
        ...
        if awaiting_completed_signal {
            ...
            // wait until set_model/set_reasoning_effort synchronizes the effective route.
